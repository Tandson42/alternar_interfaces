#!/usr/bin/env bash
# ==============================================================================
# FORÇAR LightDM (só DM) + LIGHTDM
# Execute como: sudo bash 10_usar_lightdm.sh
# ==============================================================================
set -euo pipefail
...
DE="LightDM (só DM)"; DM="lightdm"; SESSION="xfce"
...
command -v lightdm &>/dev/null || die "$DE não instalado. Execute: sudo bash 00_instalar_tudo.sh"

# --- Verificar sessão .desktop ---
SESSION_DESKTOP=$(find /usr/share/xsessions/ -iname "xfce*.desktop" 2>/dev/null | head -1)
if [[ -z "$SESSION_DESKTOP" ]]; then
    warn "Sessão 'xfce' não encontrada em /usr/share/xsessions/"
