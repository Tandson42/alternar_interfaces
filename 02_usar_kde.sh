#!/usr/bin/env bash
# ==============================================================================
# FORÇAR KDE Plasma + SDDM
# Execute como: sudo bash 02_usar_kde.sh
# ==============================================================================
set -euo pipefail
...
DE="KDE Plasma"; DM="sddm"; SESSION="plasma"
info "Configurando $DE com $DM..."

# --- Localizar binário do DM (independente de distro) ---
DM_BIN=$(command -v sddm 2>/dev/null || true)
if [[ -z "$DM_BIN" ]]; then
    # Fallback: procurar em paths comuns
    for p in /usr/sbin/sddm /usr/bin/sddm /sbin/sddm /bin/sddm /usr/lib/sddm/sddm; do
        [[ -x "$p" ]] && DM_BIN="$p" && break
    done
fi
...
command -v plasmashell &>/dev/null || die "$DE não instalado. Execute: sudo bash 00_instalar_tudo.sh"

# --- Verificar sessão .desktop ---
SESSION_DESKTOP=$(find /usr/share/xsessions/ -iname "plasma*.desktop" 2>/dev/null | head -1)
if [[ -z "$SESSION_DESKTOP" ]]; then
    warn "Sessão 'plasma' não encontrada em /usr/share/xsessions/"
...
Relogin=false
SDDMEOF
log "SDDM: tema breeze configurado."
